package botanygrowabledyes.network;
...
public class ModuleSync {
    ...
    public static void syncConfig(List<ConfigCategory> categories) {
        ...
        BotanyGrowableDyesMod.logger.info("Syncing Config with Server");
            ...
            for (Map.Entry<String, Property> entry : serverCategory.entrySet()) {
                ...
                if (prop == null) {
                    ...
                    if (!prop.getString().equals(serverProp.getString())) {
                        ...
                        changed = true;
                        BotanyGrowableDyesMod.logger.debug("Syncing %s - %s: %s", category.getName(), prop.getName(), prop.getString());
                    }
                }
            }
        }
